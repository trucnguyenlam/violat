...
function invocation(spec) {
    return spec
        ? { method: spec.name, arguments: spec.parameters, void: spec.void, equivalent: spec.equivalent }
        : { method: null, arguments: null, void: null, equivalent: null }
}
...
function placeEquivalent(spec) {
    return function* (schema) {
        yield Object.assign({ equivalent: spec.equivalent }, schema);
    };
}

function placeLanguage(spec) {
    return function* (schema) {
        yield Object.assign({ language: spec.language }, schema);
    };
}

function placeObject(spec) {
    return function* (schema) {
        yield Object.assign({ object_name: spec.object_name }, schema);
    };
}

function placeInit(spec) {
    return function* (schema) {
        yield Object.assign({ init: spec.init }, schema);
    };
}

function placeInitParams(spec) {
    return function* (schema) {
        yield Object.assign({ init_parameters: spec.init_parameters }, schema);
    };
}
...
function generator(args) {
  return [
    ...
    placeObject(args.spec),
    placeConstructor(args.spec, args.values),
    placeInit(args.spec),
    placeInitParams(args.spec),
    placeEquivalent(args.spec),
    placeLanguage(args.spec),
