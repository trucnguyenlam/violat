...
const cxxannotate = require('./c-annotation.js');
const translate = require('./translation.js');
const cxxtranslate = require('./c-translation.js');
...
async function testMethod(args) {
  ...
  while (true) {
    ...
    let annotated;
    if (args.clang) {
        annotated = await cxxannotate(schemas);
    }
    else {
        annotated = await annotate(schemas);
    }
    debug(`got ${annotated.length} annotated schemas`);

    let translated;
    if (args.clang) {
        translated = await cxxtranslate(annotated, method.name, args.libFile);
        debug(`got ${translated.length} translated schemas`);
    }else{
        translated = await translate(annotated, method.name.capitalize());
        debug(`got ${translated.length} translated schemas`);
